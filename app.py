...
input, textarea, select {
    color: #0D1B2A !important; background-color: #F8FAFC !important;
    -webkit-text-fill-color: #0D1B2A !important;
}
input::placeholder, textarea::placeholder {
    color: #A0AEC0 !important; -webkit-text-fill-color: #A0AEC0 !important; opacity: 1 !important;
}
...
div[data-testid="stVerticalBlockBorderWrapper"] {
    ...
    border-radius: 16px !important; box-shadow: 0 2px 14px rgba(0,0,0,0.06) !important;
...
.session-card {
    display: flex; align-items: center; justify-content: space-between;
    padding: 14px 16px; background: #F8FAFC; border: 1px solid #E8EDF3;
    border-radius: 12px; margin-bottom: 8px;
}
.session-card .sc-title { font-weight: 700; font-size: 0.92rem; color: #002855; }
.session-card .sc-meta  { font-size: 0.76rem; color: #8A9BAE; margin-top: 2px; }
.user-row { display: flex; align-items: center; justify-content: space-between;
    padding: 10px 14px; background: #F8FAFC; border: 1px solid #E8EDF3;
...
# ─────────────────────────────────────────────
#  SESSION CRUD
#  data stored as JSON text — no pickle, works
#  directly with the counting HTML page
# ─────────────────────────────────────────────
def df_to_json(df):
...
def json_to_df(j):
    ...
    df["last_updated"]   = df.get("last_updated", "").fillna("").astype(str)
...
#  HELPERS
# ─────────────────────────────────────────────
def header(username="", is_admin=False):
...
with st.sidebar:
    ...
    if st.button("🚪 Sign Out", use_container_width=True):
        for k in ["user","is_admin"]: st.session_state.pop(k, None)
        st.rerun()
...
#  DASHBOARD TABS
# ─────────────────────────────────────────────
header(username=CU, is_admin=IS_ADMIN)
            ...
            if not sid_in:
                ...
                if exists:
                    ...
                    try:
                        ...
                        # Get the JSON data for the counting page
                        data_json = df_to_json(df)
                        st.success(f"Session **{sid_in}** created!")
                        st.info("👇 Click **Open Counting Page** below to start counting on your phone")
                        # Build the URL for the counting page
                        # Encode data into a link to counting.html
                        st.session_state.launch_sid      = sid_in
                        st.session_state.launch_location = loc_in
                        st.session_state.launch_data     = data_json
                        st.rerun()
                    except Exception as e:
                        st.error(f"Failed to read file: {e}")

    # Launch counting page button
    if "launch_sid" in st.session_state:
        sid_l  = st.session_state.launch_sid
        loc_l  = st.session_state.launch_location
        data_l = st.session_state.launch_data

        # Embed the counting page launcher
        data_b64 = __import__("base64").b64encode(data_l.encode()).decode()
        counting_url = f"counting.html"  # same domain

        st.markdown("---")
        section("Ready to Count")
        st.markdown(f"""
        **Session:** `{sid_l}`  
        **Location:** {loc_l or '—'}  
        **Products:** {len(json.loads(data_l))} items loaded
        """)

        # Self-contained launch — opens counting page with data in localStorage
        st.components.v1.html(f"""
        <script>
        // Store session data in localStorage so counting page can read it
        localStorage.setItem('sc_sid',      '{sid_l}');
        localStorage.setItem('sc_location', '{loc_l}');
        localStorage.setItem('sc_user',     '{CU}');
        localStorage.setItem('sc_data',     atob('{data_b64}'));
        localStorage.setItem('sc_counter',  '0');
        </script>
        """, height=0)

        st.markdown(f"""
        <a href="/counting" target="_blank" style="
            display:block; text-align:center;
            background: linear-gradient(135deg,#002855,#00509E);
            color:#fff; text-decoration:none; border-radius:12px;
            padding:16px; font-weight:700; font-size:1rem;
            box-shadow:0 4px 16px rgba(0,80,158,0.3); margin-top:8px;">
            📱 Open Counting Page →
        </a>
        """, unsafe_allow_html=True)
            ...
            for _, row in my.iterrows():
                ...
                with st.container(border=True):
                    ...
                    with c1:
                        ...
                        """, unsafe_allow_html=True)
                    with c2:
                        if st.button("▶ Resume", key=f"res_{row['sid']}", use_container_width=True):
                            data_json = get_session_json(row["sid"])
                            data_b64  = __import__("base64").b64encode(data_json.encode()).decode()
                            st.session_state.launch_sid      = row["sid"]
                            st.session_state.launch_location = row["location"] or ""
                            st.session_state.launch_data     = data_json
                            st.components.v1.html(f"""
                            <script>
                            localStorage.setItem('sc_sid',      '{row["sid"]}');
                            localStorage.setItem('sc_location', '{row["location"] or ""}');
                            localStorage.setItem('sc_user',     '{CU}');
                            localStorage.setItem('sc_data',     atob('{data_b64}'));
                            localStorage.setItem('sc_counter',  '0');
                            </script>
                            """, height=0)
                            st.success(f"Session loaded! Open the counting page.")
            ...
            with st.container(border=True):
                ...
                for _, u in get_all_users().iterrows():
                    ...
                    with uc1:
                        ...
                        st.markdown(f'<div class="user-row"><div><div class="user-name">{u["username"]}</div><div class="user-meta">{role} · Joined {u["created"] or "—"}</div></div></div>', unsafe_allow_html=True)
                    with uc2:
                        if u["username"] != CU:
                ...
                if all_s.empty:
                    ...
                    for _, row in all_s.iterrows():
                        ...
                        <div class="session-card">
                          <div>
                            <div class="sc-title">{row['sid']}</div>
                            <div class="sc-meta">👤 {row['username']} · 📍 {row['location'] or '—'} · 🕐 {row['updated'] or '—'}</div>
                          </div>
                        </div>""", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
#  SYNC ENDPOINT
#  The counting HTML page POSTs updated JSON
#  back here via fetch(). We read it from query
#  params and save to DB.
# ─────────────────────────────────────────────
qp = st.query_params
if qp.get("action") == "sync":
    ...
    if sid_s and data_s:
        try:
            df_sync = json_to_df(data_s)
            row     = run("SELECT username, location FROM stock_sessions WHERE sid=%s", (sid_s,), fetch="one")
            if row:
                save_session(sid_s, row["username"], row["location"] or "", df_sync)
        except Exception:
            pass
